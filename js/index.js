...
function loadEducations(educations){
	...
	for(i=0;i<educations.length;i++){
		...
		education = '<div class="row education"><div class="col m6 s12">					<div class="row title"><hr></div><div class="row">'+educations[i].periodStart+'-'+educations[i].periodEnd+'</div><div class="row">'+educations[i].institute+'</div>		</div><div class="col m6 s12 details"><ul class="collapsible" data-collapsible="accordion"><li><div class="collapsible-header"><i class="material-icons">view_list</i>Completed following more details</div><div class="collapsible-body">';
		var courses = educations[i].detail;
		courses.sort(function(a,b){
			return a.sn-b.sn;
		});
		var coursesInnerHTML = '';
		for(j=0;j<courses.length;j++){
			coursesInnerHTML+='<div class="row"><div class="row">'+courses[j].degree+'</div><div class="row">'+courses[j].major
				+'</div><div class="row">'+courses[j].gpa+'</div></div>';
		}
